# uvicorn suggestUniversalPersonalities:app --reload
...
@app.get("/suggestUniversalPersonalities/",operation_id="suggestUniversalPersonalities")
def read_item(model:str,progress_bar:bool,country_names: List[str] = Query(...)):
    num_cores = max(min(multiprocessing.cpu_count() // 2, 2),1)
    results = [{"country_name" : country_name, "model" : model} for country_name in list(set(country_names))]
    with WorkerPool(n_jobs=num_cores,daemon=False) as pool:
        results = pool.map(run_suggestUniversalPersonality_execution, results, progress_bar=progress_bar)
    return results


if __name__=="__main__":
    mcp = FastApiMCP(app,include_operations = ["suggestUniversalPersonalities"])
    mcp.mount_http()
    import uvicorn
