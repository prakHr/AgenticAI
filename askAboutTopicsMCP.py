# uvicorn askAboutTopics:app --reload
...
@app.get("/askAboutTopics/",operation_id="get_topics")
def read_item(model:str,progress_bar:bool,topic_names:List[str] = Query(...)):
    ...
    with WorkerPool(n_jobs=num_cores,daemon=False) as pool:
        results = pool.map(run_askTopicDetails_execution, results, progress_bar=progress_bar)
    return results


if __name__=="__main__":
    mcp = FastApiMCP(app,include_operations = ["get_topics"])
    mcp.mount_http()
    import uvicorn
