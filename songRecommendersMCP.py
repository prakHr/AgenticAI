# uvicorn songRecommenders:app --reload
...
@app.get("/songRecommenders/",operation_id="recommend_songs")
def read_item(model:str,progress_bar:bool,song_genres: List[str] = Query(...)):
    num_cores = max(min(multiprocessing.cpu_count() // 2, 2),1)
    results = [{"song_genre" : song_genre, "model" : model} for song_genre in list(set(song_genres))]
    with WorkerPool(n_jobs=num_cores,daemon=False) as pool:
        results = pool.map(run_songSuggestion_execution, results, progress_bar=progress_bar)
    return results
